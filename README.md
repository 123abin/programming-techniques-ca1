# Programming Techniques CA1 – Question 1

Name: Abin Sebastian

Student ID: 20089590

## Project
Churros Ordering System

## Description:
This program is a simple churros ordering system created using C#. 
It allows a user to place churros orders and deliver them using a queue data structure.

## Features:
- Display churros menu
- Place an order
- Select churros type
- Enter quantity
- Calculate total bill
- Deliver orders in the order they were placed

## How the Program Works:
1. The program shows a menu with options.
2. The user can place an order by choosing a churros type and quantity.
3. The order is added to a queue.
4. When delivering an order, the first order in the queue is delivered.(FIFO-First In First Out)

## Files in the Project:
- **Program.cs** – Contains the main program, Order class, and Churros class.
- **Screenshots** – Contains screenshots of the program output.
